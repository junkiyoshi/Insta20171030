...
void ofApp::update(){
	...
	this->depth.copyTo(this->depth_log[ofGetFrameNum() % 7]);
}

//--------------------------------------------------------------
void ofApp::draw(){

	if (ofGetFrameNum() <= 7) { return; }
	...
	for (int i = 0; i < 7; i++) {
		ofColor c;
		c.setHsb(216 - i * 35, 255, 255);
		ofSetColor(c);
		for (int y = 0; y < this->depth_log[i].rows; y += 1) {
			for (int x = 0; x < this->depth_log[i].cols; x += 1) {
				int z = depth_log[i].at<unsigned short>(y, x);
				if (z != 0 && z < 1024) {
					ofDrawRectangle(x, y, 1.5, 1.5);
				}
			}
		}
	}
